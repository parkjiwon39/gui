...
public class MenuTest extends JFrame implements ActionListener{
	...
	private JMenuItem mnNew,mnOpen,mnSave,mnExit,mnCancel,mnCut,mnCopy,mnAll,mnDate,mnPaste;
		...
		if(menu==mnNew) {
			...
			if(retVal==0) {//열기 버튼 클릭한 경우
				...
				try(BufferedReader reader=new BufferedReader
						(new InputStreamReader(new FileInputStream(file.getPath()),"ms949"))){
					String msg;
			...
			//textArea.insert(format.format(new Date()), textArea.getCaretPosition());위랑 같음
		}
	...
	private void save() {
		...
		if(retVal==0) {//저장
			...
			(new OutputStreamWriter(new FileOutputStream(file.getPath())))){
